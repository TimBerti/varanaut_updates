...
def calculate_metrics(db):
    '''
    Calculates annual financial metrics and updates db.
    '''
    ...
    sql = f'''
        ...
        SET
            ...
            price_cash_flow = market_cap / NULLIF(free_cashflow, 0),
            price_book = market_cap / NULLIF(total_assets - total_liabilities, 0),
            dividend_yield = -100 * dividends_paid / NULLIF(market_cap, 0),
            payout_ratio = 100 * dividends_paid / NULLIF(net_income, 0),
            gross_profit_margin = 100 * gross_profit / NULLIF(total_revenue, 0),
            net_profit_margin = 100 * net_income / NULLIF(total_revenue, 0),
            ...
            debt_to_revenue = total_liabilities / NULLIF(total_revenue, 0),
    ...
    print(f'Finished calculating annual metrics. ({end - start:.2f}s)')
    ...
    sql = f'''
        WITH cte AS (	
            SELECT
                ticker,
                time,
                LAG(total_revenue, 1) OVER w revenue_1,
                LAG(total_revenue, 3) OVER w revenue_3,
                LAG(total_revenue, 5) OVER w revenue_5,
                LAG(total_revenue, 9) OVER w revenue_9,
                LAG(net_income, 1) OVER w net_income_1,
                LAG(net_income, 3) OVER w net_income_3,
                LAG(net_income, 5) OVER w net_income_5,
                LAG(net_income, 9) OVER w net_income_9,
                LAG(dividends_paid, 1) OVER w dividend_1,
                LAG(dividends_paid, 3) OVER w dividend_3,
                LAG(dividends_paid, 5) OVER w dividend_5,
                LAG(dividends_paid, 9) OVER w dividend_9
        ...
        UPDATE companies_annual c
        SET
            price_earnings_growth = c.price_earnings / NULLIF(100 * (c.net_income / NULLIF(cte.net_income_1, 0) - 1), 0),
            price_sales_growth = c.price_sales / NULLIF(100 * (c.total_revenue / NULLIF(cte.revenue_1, 0) - 1), 0),
            revenue_growth_1y = 100 * ((CASE WHEN c.total_revenue > 0 THEN c.total_revenue ELSE NULL END) / (CASE WHEN cte.revenue_1 > 0 THEN cte.revenue_1 ELSE NULL END) - 1),
            revenue_growth_3y = 100 * (((CASE WHEN c.total_revenue > 0 THEN c.total_revenue ELSE NULL END) / (CASE WHEN cte.revenue_3 > 0 THEN cte.revenue_3 ELSE NULL END)) ^ (1.0/3) - 1),
            revenue_growth_5y = 100 * (((CASE WHEN c.total_revenue > 0 THEN c.total_revenue ELSE NULL END) / (CASE WHEN cte.revenue_5 > 0 THEN cte.revenue_5 ELSE NULL END)) ^ (1.0/5) - 1),
            revenue_growth_9y = 100 * (((CASE WHEN c.total_revenue > 0 THEN c.total_revenue ELSE NULL END) / (CASE WHEN cte.revenue_9 > 0 THEN cte.revenue_9 ELSE NULL END)) ^ (1.0/9) - 1),
            earnings_growth_1y = 100 * ((CASE WHEN c.net_income > 0 THEN c.net_income ELSE NULL END) / (CASE WHEN cte.net_income_1 > 0 THEN cte.net_income_1 ELSE NULL END) - 1),
    ...
    sql = '''
        WITH cte AS (
            SELECT 
                ...
                sector,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), market_cap IS NOT NULL) 
                    ORDER BY market_cap
                ) market_cap_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), price_earnings IS NOT NULL)
                ...
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), price_sales IS NOT NULL)
                    ORDER BY price_sales DESC
                ) price_sales_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), price_sales_growth IS NOT NULL)
                    ORDER BY price_sales_growth DESC
                ) price_sales_growth_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), price_cash_flow IS NOT NULL)
                    ORDER BY price_cash_flow DESC
                ) price_cash_flow_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), price_book IS NOT NULL)
                    ORDER BY price_book DESC
                ) price_book_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), dividend_yield IS NOT NULL)
                    ORDER BY dividend_yield
                ) dividend_yield_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), payout_ratio IS NOT NULL)
                    ORDER BY payout_ratio
                ) payout_ratio_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), revenue_growth_1y IS NOT NULL)
                    ORDER BY revenue_growth_1y
                ) revenue_growth_1y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), earnings_growth_1y IS NOT NULL)
                    ORDER BY earnings_growth_1y
                ) earnings_growth_1y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), dividend_growth_1y IS NOT NULL)
                    ORDER BY dividend_growth_1y
                ) dividend_growth_1y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), revenue_growth_3y IS NOT NULL)
                    ORDER BY revenue_growth_3y
                ) revenue_growth_3y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), earnings_growth_3y IS NOT NULL)
                    ORDER BY earnings_growth_3y
                ) earnings_growth_3y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), dividend_growth_3y IS NOT NULL)
                    ORDER BY dividend_growth_3y
                ) dividend_growth_3y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), revenue_growth_5y IS NOT NULL)
                    ORDER BY revenue_growth_5y
                ) revenue_growth_5y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), earnings_growth_5y IS NOT NULL)
                    ORDER BY earnings_growth_5y
                ) earnings_growth_5y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), dividend_growth_5y IS NOT NULL)
                    ORDER BY dividend_growth_5y
                ) dividend_growth_5y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), revenue_growth_9y IS NOT NULL)
                    ORDER BY revenue_growth_9y
                ) revenue_growth_9y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), earnings_growth_9y IS NOT NULL)
                    ORDER BY earnings_growth_9y
                ) earnings_growth_9y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), dividend_growth_9y IS NOT NULL)
                    ORDER BY dividend_growth_9y
                ) dividend_growth_9y_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), gross_profit_margin IS NOT NULL)
                    ORDER BY gross_profit_margin
                ) gross_profit_margin_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), net_profit_margin IS NOT NULL)
                    ORDER BY net_profit_margin
                ) net_profit_margin_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), operating_cash_flow_margin IS NOT NULL)
                    ORDER BY operating_cash_flow_margin
                ) operating_cash_flow_margin_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), operating_margin IS NOT NULL)
                    ORDER BY operating_margin
                ) operating_margin_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), return_on_assets IS NOT NULL)
                ...
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), cash_flow_on_assets IS NOT NULL)
                    ORDER BY cash_flow_on_assets
                ) cash_flow_on_assets_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), return_on_equity IS NOT NULL)
                    ORDER BY return_on_equity
                ) return_on_equity_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), revenue_on_equity IS NOT NULL)
                ...
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), debt_to_ebit IS NOT NULL)
                    ORDER BY debt_to_ebit DESC
                ) debt_to_ebit_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), debt_to_equity IS NOT NULL)
                    ORDER BY debt_to_equity DESC
                ) debt_to_equity_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), debt_to_revenue IS NOT NULL)
                    ORDER BY debt_to_revenue DESC
                ) debt_to_revenue_rel,
                PERCENT_RANK() OVER(
                    PARTITION BY (sector, EXTRACT(YEAR FROM time), current_ratio IS NOT NULL)
...
def calculate_change(db):
    '''
    Calculates annual change of metrics and updates db.
    '''

    print('Calculating annual changes...')
    ...
    sql = '''
        WITH cte AS (	
            SELECT
                ticker,
                time,
                LAG(market_cap, 1) OVER w market_cap_1,
                LAG(market_cap_ranker, 1) OVER w market_cap_ranker_1,
                LAG(price_earnings, 1) OVER w price_earnings_1,
                ...
                LAG(price_sales_ranker, 1) OVER w price_sales_ranker_1,
                LAG(price_sales_growth, 1) OVER w price_sales_growth_1,
                LAG(price_sales_growth_ranker, 1) OVER w price_sales_growth_ranker_1,
                LAG(price_cash_flow, 1) OVER w price_cash_flow_1,
                LAG(price_cash_flow_ranker, 1) OVER w price_cash_flow_ranker_1,
                LAG(price_book, 1) OVER w price_book_1,
                LAG(price_book_ranker, 1) OVER w price_book_ranker_1,
                LAG(dividend_yield, 1) OVER w dividend_yield_1,
                LAG(dividend_yield_ranker, 1) OVER w dividend_yield_ranker_1,
                LAG(payout_ratio, 1) OVER w payout_ratio_1,
                LAG(payout_ratio_ranker, 1) OVER w payout_ratio_ranker_1,
                LAG(revenue_growth_1y, 1) OVER w revenue_growth_1y_1,
                LAG(revenue_growth_1y_ranker, 1) OVER w revenue_growth_1y_ranker_1,
                LAG(earnings_growth_1y, 1) OVER w earnings_growth_1y_1,
                LAG(earnings_growth_1y_ranker, 1) OVER w earnings_growth_1y_ranker_1,
                LAG(dividend_growth_1y, 1) OVER w dividend_growth_1y_1,
                LAG(dividend_growth_1y_ranker, 1) OVER w dividend_growth_1y_ranker_1,
                LAG(revenue_growth_3y, 1) OVER w revenue_growth_3y_1,
                LAG(revenue_growth_3y_ranker, 1) OVER w revenue_growth_3y_ranker_1,
                LAG(earnings_growth_3y, 1) OVER w earnings_growth_3y_1,
                LAG(earnings_growth_3y_ranker, 1) OVER w earnings_growth_3y_ranker_1,
                LAG(dividend_growth_3y, 1) OVER w dividend_growth_3y_1,
                LAG(dividend_growth_3y_ranker, 1) OVER w dividend_growth_3y_ranker_1,
                LAG(revenue_growth_5y, 1) OVER w revenue_growth_5y_1,
                LAG(revenue_growth_5y_ranker, 1) OVER w revenue_growth_5y_ranker_1,
                LAG(earnings_growth_5y, 1) OVER w earnings_growth_5y_1,
                LAG(earnings_growth_5y_ranker, 1) OVER w earnings_growth_5y_ranker_1,
                LAG(dividend_growth_5y, 1) OVER w dividend_growth_5y_1,
                LAG(dividend_growth_5y_ranker, 1) OVER w dividend_growth_5y_ranker_1,
                LAG(revenue_growth_9y, 1) OVER w revenue_growth_9y_1,
                LAG(revenue_growth_9y_ranker, 1) OVER w revenue_growth_9y_ranker_1,
                LAG(earnings_growth_9y, 1) OVER w earnings_growth_9y_1,
                LAG(earnings_growth_9y_ranker, 1) OVER w earnings_growth_9y_ranker_1,
                LAG(dividend_growth_9y, 1) OVER w dividend_growth_9y_1,
                LAG(dividend_growth_9y_ranker, 1) OVER w dividend_growth_9y_ranker_1,
                LAG(gross_profit_margin, 1) OVER w gross_profit_margin_1,
                LAG(gross_profit_margin_ranker, 1) OVER w gross_profit_margin_ranker_1,
                LAG(net_profit_margin, 1) OVER w net_profit_margin_1,
                LAG(net_profit_margin_ranker, 1) OVER w net_profit_margin_ranker_1,
                LAG(operating_cash_flow_margin, 1) OVER w operating_cash_flow_margin_1,
                LAG(operating_cash_flow_margin_ranker, 1) OVER w operating_cash_flow_margin_ranker_1,
                LAG(operating_margin, 1) OVER w operating_margin_1,
                ...
                LAG(cash_flow_on_assets_ranker, 1) OVER w cash_flow_on_assets_ranker_1,
                LAG(return_on_equity, 1) OVER w return_on_equity_1,
                LAG(return_on_equity_ranker, 1) OVER w return_on_equity_ranker_1,
                LAG(revenue_on_equity, 1) OVER w revenue_on_equity_1,
                ...
                LAG(debt_to_ebit_ranker, 1) OVER w debt_to_ebit_ranker_1,
                LAG(debt_to_equity, 1) OVER w debt_to_equity_1,
                LAG(debt_to_equity_ranker, 1) OVER w debt_to_equity_ranker_1,
    ...
    print(f'Finished calculating annual changes. ({end - start:.2f}s)')


def update_annual_figures(db):
    '''
    Updates all annual figures in db.
    '''
